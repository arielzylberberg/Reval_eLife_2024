    ...
    methods
        function obj = publish_plot(n_filas,n_col,varargin)
            ...
            elseif obj.data.n_filas==2 && obj.data.n_col==1
                set(gcf,'Position',[556  230  355  582])
        ...
        function h = ylabel(obj,str, hax)
            if nargin==2
                hax = 1:length(obj.h_ax);
            end
            for i=1:length(hax)
                obj.current_ax(hax(i));
                h(i) = ylabel(str);
            end
        end
        
        function h = xlabel(obj,str, hax)
            if nargin==2
                hax = 1:length(obj.h_ax);
            end
            for i=1:length(hax)
        ...
        function same_ylim_by_row(obj)
            for i = 1:length(obj.h_ax)
                pos(i,:) = get(obj.h_ax(i),'position');
            end
            [~,~,idx] = unique(pos(:,2));
            n = nanmax(idx);
            for i=1:n
                I = idx==i;
                same_ylim(obj.h_ax(I));
            end
        end

        function same_xlim_by_column(obj)
            for i = 1:length(obj.h_ax)
                pos(i,:) = get(obj.h_ax(i),'position');
            end
            [~,~,idx] = unique(pos(:,1));
            ...
            for i=1:n
                I = idx==i;
                same_xlim(obj.h_ax(I));
            end
        end

        function tight_ylim_by_row(obj, slack_prop)
            
            if nargin<2 || isempty(slack_prop)
                slack_prop = 0.05;
            end

            % first make all tight
%             set(obj.h_ax, 'YLimSpec', 'Tight'); % NOT WORKING
            for i = 1:length(obj.h_ax)
                obj.current_ax(i);
                ylim('tight');
            end
            ...
            for i=1:n
                I = idx==i;
                a = get(obj.h_ax(I),'ylim');
                yli = cat(1,a{:});
                new_yli = [min(yli(:,1)),max(yli(:,2))];
                slack = diff(new_yli)*slack_prop;
                new_yli = new_yli + [-slack, slack];
                set(obj.h_ax(I),'ylim',new_yli);
            end
        end
        ...
        function [I1,I2] = unlabel_center_plots(obj,varargin)
        ...
        function load_from_fig(obj,figure_filename_or_hfig)
            h_fig_old = obj.h_fig;
            if ~isprop(figure_filename_or_hfig,'Position') % just to know the type of input
                h_fig   = open(figure_filename_or_hfig);
            else
                h_fig = figure_filename_or_hfig;
            end
        ...
        function rect = rect_from_axes(obj, axs)
            h = obj.h_ax(axs);
            for i=1:length(h)
                position(i,:) = get(h(i),'Position');
            end
            x = min(position(:,1));
            y = min(position(:,2));
            w = max(position(:,1)+position(:,3)) - x;
            h = max(position(:,2)+position(:,4)) - y;
            rect = [x,y,w,h];
        end
        
        
        function same_xscale(obj,h)
            if nargin==1
                h = 1:length(obj.h_ax);
            end
        ...
        function ids = new_axes_in_rect(obj,rect,num_axes,separation,dim)
            ...
            ids = length(obj.h_ax):-1:(length(obj.h_ax)-num_axes+1);
        ...
        function append_to_pdf(obj,filename,condition_to_create_new, do_save)
            if nargin<3
                condition_to_create_new = false;
                do_save = 1;
            elseif nargin==3
                do_save = 1;
            end
...
%             exportgraphics(gca,'plot2.pdf','BackgroundColor','none')
            if do_save
                if condition_to_create_new
                    % export_fig(obj.h_fig,'-pdf',filename);
                    exportgraphics(obj.h_fig,[filename,'.pdf'],'ContentType','vector','BackgroundColor','none');
                else
                    %export_fig(obj.h_fig,'-pdf',filename,'-append');
                    exportgraphics(obj.h_fig,[filename,'.pdf'],'Append',true,'ContentType','vector','BackgroundColor','none');
                end
            end
        ...
        function xtickangle(obj,ax_num,angle)
            if nargin==2
                angle = 0;
            elseif nargin==1
                angle = 0;
            ...
            for i=1:length(ax_num)
                if isinteger(ax_num(i))
                    ax = obj.h_ax(ax_num(i));
                else
                    ax = ax_num(i);
                end
                xtickangle(ax,angle);
        ...
        function resize_vertical(obj,ax_num,vert_ratio,vert_separation)
            ...
            pos = pos(ind,:);
            ax_num = ax_num(ind);
            vert_ratio = vert_ratio(ind);
            
            % new vert sizes
            
            h = diff([min(pos(:,2)), max(pos(:,2)+pos(:,4))]); % available space
            h = h - vert_separation*(length(ax_num)-1); % space between axes
            if h<0
                error('reduce vert_separation');
            end
            newV = h * vert_ratio(:)/sum(vert_ratio);
...
%         function h = xlabel(obj,ax_num,str)
%             n = length(ax_num);
%             h = nan(n,1);
%             for i=1:n
%                 obj.current_ax(ax_num(i));
%                 h(i) = xlabel(str);
%             end
%         end
        ...
        function empty_plots_set_invisible(obj)
            % sets empty plots invisi
            for i=1:length(obj.h_ax)
                if isempty(get(obj.h_ax(i), 'Children'))
                    set(obj.h_ax(i),'visible','off');
                end
            end
            
        end

        function delete_empty_plots(obj)
            % just a better name for the same function
            obj.empty_plots_set_invisible(); 
        end
        ...
        function copy_from_ax(obj,h_from,h_to,closefig_flag)
            ...
            props = {'xlim','ylim','xlabel','ylabel','yaxislocation',...
                'xscale','yscale','xtick','xticklabel','visible','title'};
            for i=1:length(props)
        ...
        function out = current_ax(obj,varargin)
            if length(varargin)==1
                i = varargin{1};
                if mod(i,1)==0
...
%                 disp('error en input')
            ...
            out = find(obj.h_ax == get(obj.h_fig,'CurrentAxes'));
        ...
        function savefig(obj, filename)
            exportgraphics(obj.h_fig,[filename,'.pdf'],'ContentType','vector');
        end
...
%             MarkerSize = [];
            ...
            LineWidthAxes = 0.5;
            LineWidthPlot = 1;
            MarkerSize = [15, 7]; % for '.' and 'o' respectively
            interpreter = '';
            for i=1:length(varargin)
                ...
                elseif isequal(varargin{i},'MarkerSize')
                    ms = varargin{i+1};
                    if numel(ms)==1
                        MarkerSize = repmat(ms,1,2); % same for '.' and 'o'
                    else
                        MarkerSize = ms;
                    end
            ...
            if not(isempty(LineWidthPlot)) && ~isnan(LineWidthPlot)
                for j=1:length(obj.h_ax)
                    ch = get(obj.h_ax(j),'Children');
                    chi = findall(ch,'Type','Line');
                    set(chi,'LineWidth',LineWidthPlot);
                    
                    chi = findall(ch,'Type','errorbar');
                    set(chi,'LineWidth',LineWidthPlot);
                end
            end
            ...
            if not(isempty(MarkerSize))
                for j=1:length(obj.h_ax)
                    ...
                    ch_line = findall(ch,'Type','Line');
                    for k=1:length(ch_line)
                        if isequal(ch_line(k).Marker,'o') || ...
                                isequal(ch_line(k).Marker,'square')
                            set(ch_line(k),'MarkerSize',MarkerSize(2));
                        elseif isequal(ch_line(k).Marker,'.')
                            set(ch_line(k),'MarkerSize',MarkerSize(1));
                        else
                            set(ch_line(k),'MarkerSize',MarkerSize(1));
                        end
                    end

                    ch_errorbar = findall(ch,'Type','ErrorBar');
%                     set(ch_errorbar,'MarkerSize',MarkerSize);
                    for k=1:length(ch_errorbar)
                        if isequal(ch_errorbar(k).Marker,'o') || ...
                                isequal(ch_errorbar(k).Marker,'square')
                            set(ch_errorbar(k),'MarkerSize',MarkerSize(2));
                        elseif isequal(ch_errorbar(k).Marker,'.')
                            set(ch_errorbar(k),'MarkerSize',MarkerSize(1));
                        else
                            set(ch_errorbar(k),'MarkerSize',MarkerSize(1));
                        end
                    end
            ...
            set(all_text,'FontWeight','normal'); % new
            ...
            % set(obj.h_ax,'XTickLabelRotation',0); % prevents rotation of xticklabels
            ...
            % added Dec-20 2023
            set(obj.h_ax,'TickDir','out','TickLength',[0.025,0.025]);
            
        end
        
        
        function handles = get_handle_of_lines_with_markers(obj,hax)
            ...
            handles = [];
            for j=1:length(hax)
                h = get(obj.h_ax(hax(j)),'children');
                for i = 1:length(h)
                    if strcmp(get(h(i),'type'),'line') && ~strcmp(get(h(i),'Marker'),'none')
                        handles = [handles, h(i)];
                    end
                end
            end
        ...
        function handles = get_handle_of_lines(obj,hax)
        ...
        function ht = text_draw_fig(obj, str, location)
            if nargin<3 || isempty(location)
                location = 'center';
            end
            if isequal(location,'center')
                pos = [0,0.8,1,0.2];
                ht = annotation('textbox',pos,...
                    'string',str,'horizontalalignment','center',...
                    'verticalalignment','top','FitBoxToText',0,'LineStyle','none');
                set(ht,'FontSize',12,'interpreter','none');
            end
            
        end
        
        function ht = text_draw_full_fig(obj, str)
            
            pos = [0.2,0.2,0.6,0.6];
            ht = annotation('textbox',pos,...
                'string',str,'horizontalalignment','center',...
                'verticalalignment','middle','FitBoxToText',0,'LineStyle','none');
            set(ht,'FontSize',18,'interpreter','none');
            borde_en_axis(gca);
            axis off
        ...
        function ht = text_draw(obj, ha, strings, location)
          % duplicated, sort-of  
            ...
            for i=1:length(ha)
                ...
                pos = get(obj.h_ax(ha(i)),'position');
                ...
                if isequal(location,'upper_left')
                    x = pos(1);
                    y = pos(2);
                    ht(i) = annotation('textbox',pos,...
                        'string',str,'horizontalalignment','left',...
                        'verticalalignment','top','FitBoxToText',1,'LineStyle','none');
                elseif isequal(location,'center')
                    x = pos(1);
                    y = pos(2);
                    ht(i) = annotation('textbox',pos,...
                        'string',str,'horizontalalignment','center',...
                        'verticalalignment','top','FitBoxToText',1,'LineStyle','none');
                elseif isequal(location,'upper_left_outside')
                    delta = pos(4);
                    new_pos = [pos(1),pos(2)+pos(4),pos(3),0.2*delta];
                    ht(i) = annotation('textbox',new_pos,...
                        'string',str,'horizontalalignment','left',...
                        'verticalalignment','bottom','FitBoxToText',1,'LineStyle','none');
                end
        ...
        function ticks_format_logplot(obj,v_hax)
            
            if nargin==1
                v_hax  = obj.h_ax;
            end
            
            for i=1:length(v_hax)
                hax = v_hax(i);
                yt = get(hax,'ytick');
                for j=1:length(yt)
        ...
        function offset_axes(obj,v_ax, sep)
            % non-touching axes
            if nargin==1
                v_ax = obj.h_ax;
                sep = 0.03;
            ...
            set(0, 'CurrentFigure', obj.h_fig);
            for i=1:length(v_ax)
                obj.current_ax(v_ax(i));
                set(gca,'tickdir','out','ticklength',[0.02,0.02]);
                offsetAxes(gca, sep); % does not work with the more logical approach
            end
        ...
        function visible_limits_until_tickmarks(obj,ax_num)
            if nargin==1 || isempty(ax_num)
                ax_num = 1:length(obj.h_ax);
            end
            for i=1:length(ax_num)
                obj.current_ax(ax_num(i));
                xticks = get(obj.h_ax(ax_num(i)),'XTick');
                yticks = get(obj.h_ax(ax_num(i)),'YTick');
                obj.visible_limits_axis(ax_num(i),1,[min(xticks),max(xticks)]);
                obj.visible_limits_axis(ax_num(i),2,[min(yticks),max(yticks)]);
            end
                
        end
        
        function visible_limits_axis(obj, ax_num, dim, range)
            
            obj.resetVert(ax_num, dim, 1, range(1) );
            obj.resetVert(ax_num, dim, 2, range(2) );
            
        end
        
        % az version, more flexible
        function resetVert(obj, ax_num, dim, below_or_above, value )

        for i=1:length(ax_num)
            obj.current_ax(ax_num(i));
            ax = gca;
            resetVert_aux(ax, dim, below_or_above, value);
            addlistener (ax, 'MarkedClean', @(obj,event)resetVert_aux(ax, dim, below_or_above, value));
            if dim==1
                xticks = get(ax,'xtick');
                if below_or_above==1
                    xticks(xticks<value) = [];
                elseif below_or_above==2
                    xticks(xticks>value) = [];
                end
...
ax.XLim(1) = ax.XLim(1) - sep*diff(ax.XLim);
ax.YLim(1) = ax.YLim(1) - sep*diff(ax.YLim);
...
function resetVert_aux(ax, dim, below_or_above, value)

% ax.XRuler.Axle.VertexData(1,1) = 0;
% ax.YRuler.Axle.VertexData(2,1) = 0;

if dim==1
    ax.XRuler.Axle.VertexData(dim,below_or_above) = value;
elseif dim==2
    ax.YRuler.Axle.VertexData(dim,below_or_above) = value;
end
