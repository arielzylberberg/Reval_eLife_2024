function [newScat, newHist] = cornerHist(scatPlot,histPlot,ratio,varargin)
...
boolExternalFigure = false;
for i = 1:length(varargin)
    if isequal(varargin,'hfig')
        boolExternalFigure = true;
...
if not(boolExternalFigure)
    f = figure;
end

set(f,'position',[250 250 600 600])
scatAx = axes('Parent',f);
set(scatAx,'Position',[0.1 0.1 0.55 0.55])
%set(scatAx,'Position',[0.1 0.1 0.35 0.35])
...
set(histAx,'Position',[.35 .35 q2 q2])
